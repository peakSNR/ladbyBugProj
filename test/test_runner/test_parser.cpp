...
TestParser::TestParser(std::string path)
    ...
    variableMap.insert({"MONAD_ROOT_DIRECTORY", Value(MONAD_ROOT_DIRECTORY)});
    variableMap.insert({"MONAD_VERSION", Value(MONAD_VERSION)});
    variableMap.insert({"MONAD_EXPORT_DB_DIRECTORY", Value(exportDBPath)});
}
...
void TestParser::genGroupName() const {
    const std::size_t subStart =
        TestHelper::appendMonadRootPath(std::string(TestHelper::E2E_TEST_FILES_DIRECTORY)).length() +
        1;
...
void TestParser::extractDataset() {
    ...
    if (datasetType == "PARQUET") {
        ...
        testGroup->dataset = currentToken.params[2];
    } else if (datasetType == "MONAD") {
        testGroup->datasetType = TestGroup::DatasetType::MONAD;
        testGroup->dataset = currentToken.params[2];
...
TestQueryResult TestParser::extractExpectedResultFromToken() {
    ...
    if (result == "ok") {
        ...
        if (line.starts_with("<FILE>:")) {
            ...
            queryResult.expectedResult.push_back(TestHelper::appendMonadRootPath(
...
TestStatement TestParser::parseStatement(const std::string& testCaseName) {
    ...
    while (true) {
        ...
        case TokenType::BATCH_STATEMENTS: {
            std::string query = paramsToString(1);
            extractConnName(query, statement);
            statement.batchStatementsCSVFile = TestHelper::appendMonadRootPath(
    ...
    while (nextLine()) {
        ...
        case TokenType::LOAD_DYNAMIC_EXTENSION: {
            ...
            loadExtensionStatement.query =
                stringFormat("LOAD EXTENSION '{}/extension/{}/build/lib{}.monad_extension'",
                    MONAD_ROOT_DIRECTORY, extensionName, extensionName);
