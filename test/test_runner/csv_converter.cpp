...
void CSVConverter::readCopyCommandsFromCSVDataset() {
    ...
    while (getline(file, line)) {
        ...
        table->csvFilePath = TestHelper::appendMonadRootPath(path.string());
...
void CSVConverter::createCopyFile() {
    ...
    if (fileExtension == ".json") {
        ...
        outfile << "load extension \"" + TestHelper::appendMonadRootPath(
                                             "extension/json/build/libjson.monad_extension\"\n");
#endif
    }
...
void CSVConverter::convertCSVFiles() {
    ...
    if (fileExtension == ".json") {
        auto result = tempConn->query(
            "load extension \"" +
            TestHelper::appendMonadRootPath("extension/json/build/libjson.monad_extension\""));
        if (!result->isSuccess()) {
