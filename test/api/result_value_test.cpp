...
static decltype(auto) copyFlatTuple(monad::processor::FlatTuple* tuple) {
    std::vector<std::unique_ptr<monad::common::Value>> ret;
...
TEST_F(ResultValueTest, getNextExample) {
    std::unique_ptr<monad::main::QueryResult> result = conn->query("MATCH (p:person) RETURN p.*");
    std::vector<std::vector<std::unique_ptr<monad::common::Value>>> tuples;
