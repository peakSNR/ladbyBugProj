...
TEST_F(WalTest, NoWALFile) {
    ...
    conn->query("COMMIT;");
    auto walFilePath = monad::storage::StorageUtils::getWALFilePath(databasePath);
...
TEST_F(WalTest, ShadowFileExistsWithoutWAL) {
    ...
    conn->query("CREATE NODE TABLE test(id INT64 PRIMARY KEY, name STRING);");
    conn->query("COMMIT;");
    auto shadowFilePath = monad::storage::StorageUtils::getShadowFilePath(databasePath);
    // Create a shadow file that is corrupted.
    std::ofstream file(shadowFilePath);
    file << "This is not a valid Monad database file.";
    file.close();
    auto walFilePath = monad::storage::StorageUtils::getWALFilePath(databasePath);
    ASSERT_TRUE(std::filesystem::exists(walFilePath));
    ASSERT_TRUE(std::filesystem::file_size(walFilePath) > 0);
...
TEST_F(WalTest, CorruptedWALChecksumMismatchInHeader) {
    ...
    EXPECT_THROW(createDBAndConn();, monad::common::StorageException);
}
...
TEST_F(WalTest, WALFileLeftoverFromPreviousDBNewReadOnlyDB) {
    ...
    auto createEmptyDB = [&]() {
        database = std::make_unique<monad::main::Database>(databasePath, *systemConfig);
    };
