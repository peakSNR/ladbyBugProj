...
public:
    ...
    void SetUp() override {
        ...
        if (datasetType == TestGroup::DatasetType::MONAD && dataset != "empty" &&
            !generateBinaryDemo) {
        ...
        if (datasetType != TestGroup::DatasetType::MONAD && dataset != "empty") {
            ...
            initGraph(TestHelper::appendMonadRootPath("dataset/demo-db/parquet/"));
        } else if (generateBinaryDemo) {
            initGraph(TestHelper::appendMonadRootPath(
    ...
    void setUpDataset() {
        ...
        case TestGroup::DatasetType::CSV_TO_PARQUET: {
            auto csvDatasetPath = TestHelper::appendMonadRootPath("dataset/" + dataset);
        ...
        default: {
            ...
            // datasets exported from earlier Monad versions, otherwise the default path.
            std::string rootDir = TestHelper::E2E_OVERRIDE_IMPORT_DIR.empty() ?
                                      "dataset/" :
                                      TestHelper::E2E_OVERRIDE_IMPORT_DIR;
            dataset = TestHelper::appendMonadRootPath(rootDir + dataset);
        }
        }
    }
...
void checkGtestParams(int argc, char** argv) {
    if (argc > 1) {
        ...
        if (argument == "--gtest_list_tests") {
            ...
            scanTestFiles(TestHelper::appendMonadRootPath(TestHelper::E2E_TEST_FILES_DIRECTORY));
        }
...
int main(int argc, char** argv) {
    try {
        ...
        if (env_import_data_dir != nullptr) {
            auto path =
                TestHelper::appendMonadRootPath(std::filesystem::path(env_import_data_dir).string());
        ...
        if (argc > 1) {
            auto path = TestHelper::appendMonadRootPath(
