...
public:
    ...
    static constexpr uint64_t HASH_INDEX_MEM =
        common::HashIndexConstants::NUM_HASH_INDEXES * common::MONAD_PAGE_SIZE * 9;
    ...
    static std::string getTestListFile() {
        return appendMonadRootPath(std::string(E2E_TEST_FILES_DIRECTORY) + "/test_list");
    }

    static std::string appendMonadRootPath(const std::string& path) {
        if (std::filesystem::path(path).is_relative()) {
            return MONAD_ROOT_DIRECTORY + std::string("/") + path;
        }
        return path;
    }
    ...
    static std::filesystem::path getRootTempDir() {
        ...
        if (tempDir != nullptr) {
            return std::filesystem::path(tempDir) / "monad";
        }
        return std::filesystem::temp_directory_path() / "monad";
    }
...
#ifdef _WIN32
        // monad still doesn't support backslashes in paths on windows
