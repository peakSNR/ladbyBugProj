...
TEST(CompressionTests, IntegerBitpackingMetadataInvalidPhysicalType) {
    ...
    monad::storage::InPlaceUpdateLocalState localUpdateState{};
    EXPECT_THROW(metadata.canUpdateInPlace(&data, 0, 1, PhysicalTypeID::ARRAY, localUpdateState),
        StorageException);

    EXPECT_THROW(metadata.numValues(MONAD_PAGE_SIZE, LogicalType::STRING()), StorageException);
...
TEST(CompressionTests, FloatCompressionMetadataInvalidPhysicalType) {
    ...
    EXPECT_THROW(metadata.numValues(MONAD_PAGE_SIZE, LogicalType::UINT64()), StorageException);
...
TEST(CompressionTests, IntegerPackingTest128AllPositive) {
    std::vector<monad::common::int128_t> src(101);

    {
        monad::common::int128_t cur = 1;
        monad::common::int128_t diff = 1;
...
TEST(CompressionTests, IntegerPackingTest128Negative) {
    ...
    {
        auto cur = -(monad::common::int128_t(1) << 120);
        monad::common::int128_t diff = 1;
        std::ranges::generate(src.begin(), src.end(), [&diff, &cur] {
            diff *= 2;
            cur += diff;
...
TEST(CompressionTests, IntegerPackingTest128NegativeSimple) {
    std::vector<monad::common::int128_t> src{-1024, -1027, -1023};
...
TEST(CompressionTests, IntegerPackingTest128CompressFullChunkLargeWidth) {
    std::vector<monad::common::int128_t> src{IntegerBitpacking<int128_t>::CHUNK_SIZE,
...
TEST(CompressionTests, IntegerPackingMultiPage64) {
    ...
    std::vector<monad::common::int128_t> src(numValues);
    const auto M = (monad::common::int128_t(1) << 126) - 1;
