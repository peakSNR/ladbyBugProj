#!/usr/bin/env python3
"""
Monad Database Migration Script

This script helps migrate Monad databases between versions.
- Sets up isolated Python environments for each Monad version
...
Usage Examples:
    ...
    python migrate-monad-db.py --old-version 0.9.0 --new-version 0.11.0 --old-db /path/to/old/database --new-db /path/to/new/database

Notes:
- Can only be used to migrate to newer Monad versions, from 0.11.0 onwards
"""
...
MONAD_FILE_EXTENSIONS = ["", ".wal", ".shadow"]


# FIXME: Replace this with a Monad query to get the mapping when available.
monad_version_mapping = {
    34: "0.7.0",
    35: "0.7.1",
    36: "0.8.2",
    37: "0.9.0",
...
minimum_monad_migration_version = "0.11.0"


def monad_version_comparison(version: str, target: str) -> bool:
    """Return True if Monad *v* is greater or equal to target version"""
...
def read_monad_storage_version(monad_db_path: str) -> int:
    """
    Reads the Monad storage version.

    :param monad_db_path: Path to the Monad database file/directory.
    :return: Storage version code as an integer.
    """
    if os.path.isdir(monad_db_path):
        monad_version_file_path = os.path.join(monad_db_path, "catalog.kz")
        if not os.path.isfile(monad_version_file_path):
            raise FileNotFoundError("Monad catalog.kz file does not exist")
    else:
        monad_version_file_path = monad_db_path

    with open(monad_version_file_path, "rb") as f:
        f.seek(4)
        # Read the next 8 bytes as a little-endian unsigned 64-bit integer
        data = f.read(8)
        if len(data) < 8:
            raise ValueError(
                f"File '{monad_version_file_path}' does not contain a storage version code."
            )
        version_code = struct.unpack("<Q", data)[0]

    if version_code in monad_version_mapping:
        return monad_version_mapping[version_code]
    else:
        raise ValueError(f"Could not map version_code {version_code} to proper Monad version.")


def ensure_env(version: str, export_dir) -> str:
    """
    Creates a venv at `{export_dir}/.monad_envs/{version}` and installs `monad=={version}`
    Returns the venv's python executable path.
    """
    # Use temp directory to create venv
    monad_envs_dir = os.path.join(export_dir, ".monad_envs")

    # venv base under the script directory
    base = os.path.join(monad_envs_dir, version)
    py_bin = os.path.join(base, "bin", "python")
    # If environment already exists clean it
    if os.path.isdir(base):
        shutil.rmtree(base)

    print(f"→ Setting up venv for Monad {version}...", file=sys.stderr)
    # Create venv
    ...
    # Install the specific Monad version
    subprocess.run([py_bin, "-m", "pip", "install", "--upgrade", "pip"], check=True)
    subprocess.run([py_bin, "-m", "pip", "install", f"monad=={version}"], check=True)
    return py_bin


def run_migration_step(python_exe: str, db_path: str, cypher: str):
    """
    Uses `python_exe` to connect to the Monad database at `db_path` and run the `cypher` query.
    """
    snippet = f"""
import monad
db = monad.Database(r"{db_path}")
conn = monad.Connection(db)
...
def monad_migration(
    ...
    if not monad_version_comparison(
        version=new_version, target=minimum_monad_migration_version
    ):
        raise ValueError(
            f"New version for monad is not supported, has to be equal or higher than version: {minimum_monad_migration_version}"
        )

    print(
        f"Migrating Monad database from {old_version} to {new_version}", file=sys.stderr
    )
    print(f"Source: {old_db}", file=sys.stderr)
    print("", file=sys.stderr)

    # If version of old monad db is not provided try to determine it based on file info
    if not old_version:
        old_version = read_monad_storage_version(old_db)
    ...
    with tempfile.TemporaryDirectory() as export_dir:
        # Set up environments
        print(f"Setting up Monad {old_version} environment...", file=sys.stderr)
        old_py = ensure_env(old_version, export_dir)
        print(f"Setting up Monad {new_version} environment...", file=sys.stderr)
        new_py = ensure_env(new_version, export_dir)

        export_file = os.path.join(export_dir, "monad_export")
    ...
    # Rename new monad database to old monad database name if enabled
    if overwrite or delete_old:
        # Remove monad lock from migrated DB
        lock_file = new_db + ".lock"
        if os.path.exists(lock_file):
            os.remove(lock_file)
        rename_databases(old_db, old_version, new_db, delete_old)

    print("Monad graph database migration finished successfully!")
...
def rename_databases(old_db: str, old_version: str, new_db: str, delete_old: bool):
    ...
    if os.path.isfile(old_db):
        ...
        for ext in MONAD_FILE_EXTENSIONS:
            src = old_db + ext
            dst = backup_base + ext
        ...
        # Directory-based Monad database
...
def main():
    p = argparse.ArgumentParser(
        description="Migrate Monad DB via PyPI versions",
        epilog="""
Examples:
  %(prog)s --old-version 0.9.0 --new-version 0.11.0 \\
    --old-db /path/to/old/db --new-db /path/to/new/db --overwrite

Note: This script will create temporary virtual environments in .monad_envs/ directory
to isolate different Monad versions.
        """,
    ...
    p.add_argument(
        ...
        required=False,
        default=None,
        help="Source Monad version (e.g., 0.9.0). If not provided, automatic monad version detection will be attempted.",
    )
    p.add_argument(
        "--new-version", required=True, help="Target Monad version (e.g., 0.11.0)"
    )
