...
# bigger.
def get_version(monad_root):
    cmake_file = os.path.join(monad_root, "CMakeLists.txt")
    with open(cmake_file) as f:
        for line in f:
            if line.startswith("project(Monad VERSION"):
...
def export_datasets_and_test(
    ...
    if not os.path.exists(export_path + os.sep):
        ...
        export_script_path = os.path.join(monad_root, "scripts", "export-dbs.py")
        exec_path = os.path.join(
            base_worktree, "build", "relwithdebinfo", "tools", "shell", "monad"
        )
        run_command(
...
def write_split_testfile(
    ...
    def replace_placeholders(lines):
        return [
            line.replace("${MONAD_EXPORT_DB_DIRECTORY}", os.path.join(db_dir, ""))
            for line in lines
        ]
    ...
    with open(export_path, "w") as f:
        f.write(header.replace("${MONAD_EXPORT_DB_DIRECTORY}", os.path.join(db_dir, "")))
        f.writelines(replace_placeholders(export_lines))

    with open(import_path, "w") as f:
        f.write(
            transform_import_header(
                header.replace("${MONAD_EXPORT_DB_DIRECTORY}", os.path.join(db_dir, ""))
            )
        )
...
def run_export_specific_tests(
    monad_root, base_worktree, test_worktree, test_dir, output_dir, cleanup
):
    # Split tests in test_dir
    split_files(test_dir, output_dir)
    # Build base_worktree monad
...
def main():
    ...
    try:
        ...
        monad_root = os.path.abspath(os.path.join(script_dir, ".."))
        base_worktree = os.path.join(monad_root, ".worktree-base")
        test_worktree = os.path.join(monad_root, ".worktree-test")
        create_worktree(base_worktree, base_commit, monad_root)
        create_worktree(test_worktree, test_commit, monad_root)
        ...
        if base_worktree or test_worktree:
            ...
            if base_worktree:
                remove_worktree(base_worktree, monad_root)
            if test_worktree:
                remove_worktree(test_worktree, monad_root)
