...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        file_name = "monad-%s.tar.gz" % _get_monad_version()
    ...
    with TemporaryDirectory() as tempdir:
        subprocess.check_call(
            [
                ...
                "tar",
                "-o",
                os.path.join(tempdir, "monad-source.tar"),
                "HEAD",
            ],
            cwd="../..",
        )

        with tarfile.open(os.path.join(tempdir, "monad-source.tar")) as tar:
            tar.extractall(path=os.path.join(tempdir, "monad-source"), filter=None)

        os.remove(os.path.join(tempdir, "monad-source.tar"))

        # Remove components that are not needed for the pip package
        shutil.rmtree(os.path.join(tempdir, "monad-source/dataset"))
        shutil.rmtree(os.path.join(tempdir, "monad-source/examples"))
        shutil.rmtree(os.path.join(tempdir, "monad-source/benchmark"))
        shutil.rmtree(os.path.join(tempdir, "monad-source/logo"))
        shutil.rmtree(os.path.join(tempdir, "monad-source/extension"))
        shutil.rmtree(os.path.join(tempdir, "monad-source/test"))
        shutil.rmtree(os.path.join(tempdir, "monad-source/.github"))

        os.makedirs(os.path.join(tempdir, "monad"))
        ...
        shutil.copy2(
            os.path.join(tempdir, "monad.egg-info", "PKG-INFO"),
