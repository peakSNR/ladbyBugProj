...
static LogicalType inferMonadType(nlohmann::json types) {
...
std::pair<std::string, std::string> getCreateNodeTableQuery(httplib::Client& cli,
    ...
    for (const auto& item : data) {
        ...
        auto kuType = inferMonadType(item["row"][1]);
...
std::string migrateQuery(ClientContext& /*context*/, const TableFuncBindData& bindData) {
    ...
    outputQuery.append("] as row RETURN row[1] as monad_table, row[2] as monad_property, row[3] as "
                       "monad_type, row[4] as neo4j_types;");
    result += outputQuery;
    return result;
}
