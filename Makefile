...
ifdef PAGE_SIZE_LOG2
	CMAKE_FLAGS += -DMONAD_PAGE_SIZE_LOG2=$(PAGE_SIZE_LOG2)
endif

ifdef DEFAULT_REL_STORAGE_DIRECTION
	CMAKE_FLAGS += -DMONAD_DEFAULT_REL_STORAGE_DIRECTION=$(DEFAULT_REL_STORAGE_DIRECTION)
endif

ifdef VECTOR_CAPACITY_LOG2
	CMAKE_FLAGS += -DMONAD_VECTOR_CAPACITY_LOG2=$(VECTOR_CAPACITY_LOG2)
endif

ifdef NODE_GROUP_SIZE_LOG2
	CMAKE_FLAGS += -DMONAD_NODE_GROUP_SIZE_LOG2=$(NODE_GROUP_SIZE_LOG2)
endif

ifdef MAX_SEGMENT_SIZE_LOG2
	CMAKE_FLAGS += -DMONAD_MAX_SEGMENT_SIZE_LOG2=$(MAX_SEGMENT_SIZE_LOG2)
endif
...
java_native_header:
	cmake --build build/$(call get-build-path,Release) --target monad_java
...
python-uv: ## Build python bindings using tools/python_api/.venv (uv, Python 3.12)
	$(MAKE) -C tools/python_api .venv
	PY_VENV=$$(cd tools/python_api && pwd)/.venv/bin/python ; \
	EXTRA_CMAKE_FLAGS="$(EXTRA_CMAKE_FLAGS) -DPython3_EXECUTABLE=$$PY_VENV -DPython_EXECUTABLE=$$PY_VENV -DPYBIND11_PYTHON_VERSION=3.12" \
		$(MAKE) GEN=$(GEN) python
...
pytest-uv: ## Build with uv venv + run python tests end-to-end
	$(MAKE) GEN=$(GEN) python-uv
	$(MAKE) GEN=$(GEN) pytest-venv
...
extension-static-link-test: extension-test-static-build
    ...
    ctest --test-dir build/$(call get-build-path,RelWithDebInfo)/test/runner --output-on-failure -j ${TEST_JOBS} --exclude-regex "${EXTENSION_TEST_EXCLUDE_FILTER}" && \
    ctest --test-dir build/$(call get-build-path,RelWithDebInfo)/extension --output-on-failure -j ${TEST_JOBS} --exclude-regex "${EXTENSION_TEST_EXCLUDE_FILTER}" && \
	aws s3 rm s3://monad-dataset-us/${RUN_ID}/ --recursive
