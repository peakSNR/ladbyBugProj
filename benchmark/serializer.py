...
from version import _get_monad_version

base_dir = os.path.dirname(os.path.realpath(__file__))

def serialize(monad_exec_path, dataset_name, dataset_path, serialized_graph_path, benchmark_copy_log_dir,
              single_thread: bool = False):
    bin_version = _get_monad_version()
    ...
    for s in serialize_queries:
        ...
        # Run monad shell one query at a time. This ensures a new process is
        ...
        process = subprocess.Popen([monad_exec_path, db_path],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Serializes dataset to a monad database')
    ...
    if sys.platform == "win32":
        default_monad_exec_path = os.path.join(
            base_dir, '..', 'build', 'release', 'tools', 'shell', 'monad_shell')
    else:
        default_monad_exec_path = os.path.join(
            base_dir, '..', 'build', 'release', 'tools', 'shell', 'monad')
    parser.add_argument("--monad-shell",
                        help="Path of the monad shell executable. Defaults to the path as built in the default release build directory",
                        default=default_monad_exec_path)
    args = parser.parse_args()

    try:
        serialize(args.monad_shell, args.dataset_name, args.dataset_path, args.serialized_graph_path,
