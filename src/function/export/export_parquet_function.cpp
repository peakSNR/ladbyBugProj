...
struct ParquetOptions {
    monad_parquet::format::CompressionCodec::type codec =
        monad_parquet::format::CompressionCodec::SNAPPY;
    ...
    void setCompression(common::Value& value) {
        ...
        if (strVal == "UNCOMPRESSED") {
            codec = monad_parquet::format::CompressionCodec::UNCOMPRESSED;
        } else if (strVal == "SNAPPY") {
            codec = monad_parquet::format::CompressionCodec::SNAPPY;
        } else if (strVal == "ZSTD") {
            codec = monad_parquet::format::CompressionCodec::ZSTD;
        } else if (strVal == "GZIP") {
            codec = monad_parquet::format::CompressionCodec::GZIP;
        } else if (strVal == "LZ4_RAW") {
            codec = monad_parquet::format::CompressionCodec::LZ4_RAW;
        } else {
