...
using namespace monad::catalog;
using namespace monad::planner;
...
std::string ClientContext::getExtensionDir() const {
    return stringFormat("{}/.monad/extension/{}/{}/", clientConfig.homeDirectory,
        MONAD_EXTENSION_VERSION, extension::getPlatform());
}
...
AttachedMonadDatabase* ClientContext::getAttachedDatabase() const {
...
void ClientContext::setDefaultDatabase(AttachedMonadDatabase* defaultDatabase_) {
...
bool ClientContext::canExecuteWriteQuery() const {
    ...
    // Note: we can only attach a remote monad database in read-only mode and only one
    // remote monad database can be attached.
    const auto dbManager = DatabaseManager::Get(*this);
    for (const auto& attachedDB : dbManager->getAttachedDatabases()) {
        if (attachedDB->getDBType() == ATTACHED_MONAD_DB_TYPE) {
            return false;
        }
    }
    return true;
