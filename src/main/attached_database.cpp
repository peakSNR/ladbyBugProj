...
void AttachedDatabase::invalidateCache() {
    if (dbType != common::ATTACHED_MONAD_DB_TYPE) {
    ...
    if (vfs->fileOrPathExists(walFilePath, context)) {
        ...
        if (walFile->getFileSize() > 0) {
            throw common::RuntimeException(common::stringFormat(
                "Cannot attach an external Monad database with non-empty wal file. Try manually "
                "checkpointing the external database (i.e., run \"CHECKPOINT;\")."));
        }
    }
}

AttachedMonadDatabase::AttachedMonadDatabase(std::string dbPath, std::string dbName,
    ...
    if (DBConfig::isDBPathInMemory(dbPath)) {
        throw common::RuntimeException("Cannot attach an in-memory Monad database. Please give a "
                                       "path to an on-disk Monad database directory.");
    }
    ...
    if (!vfs->fileOrPathExists(path, clientContext)) {
        throw common::RuntimeException(common::stringFormat(
            "Cannot attach a remote Monad database due to invalid path: {}.", path));
    }
