...
monad_state monad_database_init(const char* database_path, monad_system_config config,
    monad_database* out_database) {
    try {
...
void monad_database_destroy(monad_database* database) {
    if (database == nullptr) {
        return;
    }
...
monad_system_config monad_default_system_config() {
    SystemConfig config = SystemConfig();
    auto cSystemConfig = monad_system_config();
