...
static void validateMagicBytes(common::Deserializer& deSer) {
    ...
    std::vector<uint8_t> magicBytes(numMagicBytes, 0);
    ...
    if (memcmp(magicBytes.data(), StorageVersionInfo::MAGIC_BYTES, numMagicBytes) != 0) {
        throw common::RuntimeException(
            "Unable to open database. The file is not a valid Monad database file!");
    }
}
...
std::optional<DatabaseHeader> DatabaseHeader::readDatabaseHeader(common::FileInfo& dataFileInfo) {
    if (dataFileInfo.getFileSize() < common::MONAD_PAGE_SIZE) {
