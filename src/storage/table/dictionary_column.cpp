...
bool DictionaryColumn::canDataCommitInPlace(const SegmentState& dataState,
    ...
    if (totalStringDataAfterUpdate > dataState.metadata.getNumPages() * MONAD_PAGE_SIZE) {
...
bool DictionaryColumn::canOffsetCommitInPlace(const SegmentState& offsetState,
    ...
    auto offsetCapacity =
        offsetState.metadata.compMeta.numValues(MONAD_PAGE_SIZE, offsetColumn->getDataType()) *
