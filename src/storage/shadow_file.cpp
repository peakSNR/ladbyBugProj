...
void ShadowFile::applyShadowPages(ClientContext& context) const {
    const auto pageBuffer = std::make_unique<uint8_t[]>(MONAD_PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        ...
        dataFileInfo->writeFile(pageBuffer.get(), MONAD_PAGE_SIZE,
            record.originalPageIdx * MONAD_PAGE_SIZE);
...
void ShadowFile::replayShadowPageRecords(ClientContext& context) {
    ...
    const auto headerBuffer = std::make_unique<uint8_t[]>(MONAD_PAGE_SIZE);
    shadowFileInfo->readFromFile(headerBuffer.get(), MONAD_PAGE_SIZE, 0);
    ...
    reader->resetReadOffset((header.numShadowPages + 1) * MONAD_PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        shadowFileInfo->readFromFile(pageBuffer.get(), MONAD_PAGE_SIZE,
            shadowPageIdx * MONAD_PAGE_SIZE);
        dataFileInfo->writeFile(pageBuffer.get(), MONAD_PAGE_SIZE,
            record.originalPageIdx * MONAD_PAGE_SIZE);
        shadowPageIdx++;
    }
...
void ShadowFile::flushAll(main::ClientContext& context) const {
    ...
    writer->setFileOffset(shadowingFH->getNumPages() * MONAD_PAGE_SIZE);
