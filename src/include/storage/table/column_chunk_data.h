...
class MONAD_API ColumnChunkData {
public:
    ...
    static common::page_idx_t getNumPagesForBytes(uint64_t numBytes) {
        return (numBytes + common::MONAD_PAGE_SIZE - 1) / common::MONAD_PAGE_SIZE;
    }
    ...
    // MONAD_PAGE_SIZE == MAX_SEGMENT_SIZE, even a normal column with non-constant-compressed nulls
...
class MONAD_API InternalIDChunkData final : public ColumnChunkData {
public:
