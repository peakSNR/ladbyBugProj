...
struct ParquetReaderScanState {
    ...
    std::unique_ptr<monad_apache::thrift::protocol::TProtocol> thriftFileProto;
...
public:
    ...
    monad_parquet::format::FileMetaData* getMetadata() const { return metadata.get(); }

private:
    std::unique_ptr<monad_apache::thrift::protocol::TProtocol> createThriftProtocol(
        common::FileInfo* fileInfo_, bool prefetch_mode) {
        return std::make_unique<
            monad_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport>>(
            std::make_shared<ThriftFileTransport>(fileInfo_, prefetch_mode));
    }
    const monad_parquet::format::RowGroup& getGroup(ParquetReaderScanState& state) {
        KU_ASSERT(
    ...
    static common::LogicalType deriveLogicalType(const monad_parquet::format::SchemaElement& s_ele);
    void initMetadata();
...
private:
    ...
    std::unique_ptr<monad_parquet::format::FileMetaData> metadata;
    main::ClientContext* context;
};
