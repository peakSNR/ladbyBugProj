...
public:
    ColumnReader(ParquetReader& reader, common::LogicalType type,
        const monad_parquet::format::SchemaElement& schema, common::idx_t fileIdx,
    ...
    static std::unique_ptr<ColumnReader> createReader(ParquetReader& reader,
        common::LogicalType type, const monad_parquet::format::SchemaElement& schema,
    ...
    void decompressInternal(monad_parquet::format::CompressionCodec::type codec, const uint8_t* src,
        uint64_t srcSize, uint8_t* dst, uint64_t dstSize);
    void preparePageV2(monad_parquet::format::PageHeader& pageHdr);
    void preparePage(monad_parquet::format::PageHeader& pageHdr);
    void prepareDataPage(monad_parquet::format::PageHeader& pageHdr);
...
protected:
    const monad_parquet::format::SchemaElement& schema;
    ...
    const monad_parquet::format::ColumnChunk* chunk = nullptr;
