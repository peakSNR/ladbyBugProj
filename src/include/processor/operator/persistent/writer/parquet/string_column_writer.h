...
public:
    StringColumnWriterState(monad_parquet::format::RowGroup& rowGroup, uint64_t colIdx,
...
public:
    ...
    inline monad_parquet::format::Encoding::type getEncoding(
        BasicColumnWriterState& writerState) override {
        auto& state = reinterpret_cast<StringColumnWriterState&>(writerState);
        return state.isDictionaryEncoded() ? monad_parquet::format::Encoding::RLE_DICTIONARY :
                                             monad_parquet::format::Encoding::PLAIN;
    }
