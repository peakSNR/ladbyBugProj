...
public:
    BasicColumnWriterState(monad_parquet::format::RowGroup& rowGroup, uint64_t colIdx)
...
public:
    std::unique_ptr<ColumnWriterState> initializeWriteState(
        monad_parquet::format::RowGroup& rowGroup) override;
...
protected:
    ...
    virtual monad_parquet::format::Encoding::type getEncoding(BasicColumnWriterState& /*state*/) {
        return monad_parquet::format::Encoding::PLAIN;
    }
    ...
    void setParquetStatistics(BasicColumnWriterState& state,
        monad_parquet::format::ColumnChunk& column);
    void registerToRowGroup(monad_parquet::format::RowGroup& rowGroup);
};
