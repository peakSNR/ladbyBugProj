...
LogicalType MONAD_API inferMinimalTypeFromString(const std::string& str);
LogicalType MONAD_API inferMinimalTypeFromString(std::string_view str);
...
void MONAD_API castStringToBool(const char* input, uint64_t len, bool& result);
...
inline bool tryDoubleCast(const char* input, uint64_t len, T& result) {
    ...
    auto end = input + len;
    auto parse_result = monad_fast_float::from_chars(input, end, result);
