...
public:
     ...
     */
    MONAD_API explicit Connection(Database* database);
    /**
     * @brief Destructs the connection.
     */
    MONAD_API ~Connection();
    /**
     ...
     */
    MONAD_API void setMaxNumThreadForExec(uint64_t numThreads);
    /**
     ...
     */
    MONAD_API uint64_t getMaxNumThreadForExec();
     ...
     */
    MONAD_API std::unique_ptr<QueryResult> query(std::string_view query);

    MONAD_API std::unique_ptr<QueryResult> queryAsArrow(std::string_view query, int64_t chunkSize);
     ...
     */
    MONAD_API std::unique_ptr<PreparedStatement> prepare(std::string_view query);
     ...
     * @return the prepared statement.
     */
    MONAD_API std::unique_ptr<PreparedStatement> prepareWithParams(std::string_view query,
     ...
     * @return the result of the query.
     */
    MONAD_API std::unique_ptr<QueryResult> executeWithParams(PreparedStatement* preparedStatement,
        std::unordered_map<std::string, std::unique_ptr<common::Value>> inputParams);
    /**
     * @brief interrupts all queries currently executing within this connection.
     */
    MONAD_API void interrupt();

    /**
     * @brief sets the query timeout value of the current connection. A value of zero (the default)
     * disables the timeout.
     */
    MONAD_API void setQueryTimeOut(uint64_t timeoutInMS);
...
private:
    ...
    MONAD_API void addScalarFunction(std::string name, function::function_set definitions);
    MONAD_API void removeScalarFunction(std::string name);
