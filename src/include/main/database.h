...
struct MONAD_API SystemConfig {
     ...
     * will throw an error. Otherwise, Monad will silently ignore the failure and replay up to where
     * the error occured.
...
/**
 * @brief Database class is the main class of Monad. It manages all database components.
 */
class Database {
...
public:
     ...
     */
    MONAD_API explicit Database(std::string_view databasePath,
    ...
    MONAD_API void registerFileSystem(std::unique_ptr<common::FileSystem> fs);

    MONAD_API void registerStorageExtension(std::string name,
        std::unique_ptr<storage::StorageExtension> storageExtension);

    MONAD_API void addExtensionOption(std::string name, common::LogicalTypeID type,
        common::Value defaultValue, bool isConfidential = false);

    MONAD_API void addTransformerExtension(
