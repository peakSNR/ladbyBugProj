...
class LogicalType {
    ...
    MONAD_API LogicalType(const LogicalType& other);

public:
    LogicalType() : typeID{LogicalTypeID::ANY}, extraTypeInfo{nullptr} {
        physicalType = getPhysicalType(this->typeID);
    };
    explicit MONAD_API LogicalType(LogicalTypeID typeID, TypeCategory info = TypeCategory::INTERNAL);
    EXPLICIT_COPY_DEFAULT_MOVE(LogicalType);

    MONAD_API bool operator==(const LogicalType& other) const;
    MONAD_API bool operator!=(const LogicalType& other) const;
    ...
    MONAD_API static PhysicalTypeID getPhysicalType(LogicalTypeID logicalType,
    ...
    MONAD_API static std::vector<LogicalType> copy(const std::vector<LogicalType>& types);
    MONAD_API static std::vector<LogicalType> copy(const std::vector<LogicalType*>& types);
    ...
    static MONAD_API LogicalType DECIMAL(uint32_t precision, uint32_t scale);
    ...
    static MONAD_API LogicalType STRUCT(std::vector<StructField>&& fields);

    static MONAD_API LogicalType RECURSIVE_REL(std::vector<StructField>&& fields);

    static MONAD_API LogicalType NODE(std::vector<StructField>&& fields);

    static MONAD_API LogicalType REL(std::vector<StructField>&& fields);

    static MONAD_API LogicalType UNION(std::vector<StructField>&& fields);

    static MONAD_API LogicalType LIST(LogicalType childType);
    template<class T>
    static inline LogicalType LIST(T&& childType) {
        return LogicalType::LIST(LogicalType(std::forward<T>(childType)));
    }

    static MONAD_API LogicalType MAP(LogicalType keyType, LogicalType valueType);
    template<class T>
    ...
    static MONAD_API LogicalType ARRAY(LogicalType childType, uint64_t numElements);
    template<class T>
...
private:
    ...
    friend class monad::processor::ParquetReader;
...
class MONAD_API UDTTypeInfo : public ExtraTypeInfo {
public:
...
class MONAD_API ListTypeInfo : public ExtraTypeInfo {
public:
...
class MONAD_API ArrayTypeInfo final : public ListTypeInfo {
public:
...
struct MONAD_API LogicalTypeUtils {
