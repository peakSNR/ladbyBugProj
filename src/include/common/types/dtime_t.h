...
public:
    ...
    MONAD_API static dtime_t fromCString(const char* buf, uint64_t len);
    MONAD_API static bool tryConvertInterval(const char* buf, uint64_t len, uint64_t& pos,
        dtime_t& result);
    MONAD_API static bool tryConvertTime(const char* buf, uint64_t len, uint64_t& pos,
        dtime_t& result);

    // Convert a time object to a string in the format "hh:mm:ss"
    MONAD_API static std::string toString(dtime_t time);

    MONAD_API static dtime_t fromTime(int32_t hour, int32_t minute, int32_t second,
        int32_t microseconds = 0);

    // Extract the time from a given timestamp object
    MONAD_API static void convert(dtime_t time, int32_t& out_hour, int32_t& out_min,
        int32_t& out_sec, int32_t& out_micros);

    MONAD_API static bool isValid(int32_t hour, int32_t minute, int32_t second,
