...
public:
    ...
    MONAD_API static void addition(interval_t& result, uint64_t number, std::string specifierStr);
    MONAD_API static interval_t fromCString(const char* str, uint64_t len);
    MONAD_API static std::string toString(interval_t interval);
    MONAD_API static bool greaterThan(const interval_t& left, const interval_t& right);
    MONAD_API static void normalizeIntervalEntries(interval_t input, int64_t& months, int64_t& days,
        int64_t& micros);
    MONAD_API static void tryGetDatePartSpecifier(std::string specifier, DatePartSpecifier& result);
    MONAD_API static int32_t getIntervalPart(DatePartSpecifier specifier, interval_t timestamp);
    MONAD_API static int64_t getMicro(const interval_t& val);
    MONAD_API static int64_t getNanoseconds(const interval_t& val);
    MONAD_API static const regex::RE2& regexPattern1();
    MONAD_API static const regex::RE2& regexPattern2();
};
