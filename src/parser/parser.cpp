...
std::vector<std::shared_ptr<Statement>> Parser::parseQuery(std::string_view query,
    ...
    auto monadCypherParser = MonadCypherParser(&tokens);
    monadCypherParser.removeErrorListeners();
    monadCypherParser.addErrorListener(&parserErrorListener);
    monadCypherParser.setErrorHandler(std::make_shared<ParserErrorStrategy>());

    Transformer transformer(*monadCypherParser.ku_Statements(), std::move(transformerExtensions));
