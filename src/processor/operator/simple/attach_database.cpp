...
void AttachDatabase::executeInternal(ExecutionContext* context) {
    ...
    if (common::StringUtils::getUpper(attachInfo.dbType) == common::ATTACHED_MONAD_DB_TYPE) {
        auto db = std::make_unique<main::AttachedMonadDatabase>(attachInfo.dbPath,
            attachInfo.dbAlias, common::ATTACHED_MONAD_DB_TYPE, client);
