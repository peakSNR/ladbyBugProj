...
ParquetWriter::ParquetWriter(std::string fileName, std::vector<common::LogicalType> types,
    std::vector<std::string> columnNames, monad_parquet::format::CompressionCodec::type codec,
    ...
    monad_apache::thrift::protocol::TCompactProtocolFactoryT<ParquetWriterTransport> tprotoFactory;
    ...
    fileMetaData.created_by = "MONAD";

    fileMetaData.schema.resize(1);

    // populate root schema object
    fileMetaData.schema[0].name = "monad_schema";
    fileMetaData.schema[0].num_children = this->types.size();
    fileMetaData.schema[0].__isset.num_children = true;
    fileMetaData.schema[0].repetition_type = monad_parquet::format::FieldRepetitionType::REQUIRED;
