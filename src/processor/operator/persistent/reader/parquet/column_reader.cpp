...
using monad_parquet::format::CompressionCodec;
using monad_parquet::format::ConvertedType;
using monad_parquet::format::Encoding;
using monad_parquet::format::PageType;
using monad_parquet::format::Type;

ColumnReader::ColumnReader(ParquetReader& reader, LogicalType type,
    const monad_parquet::format::SchemaElement& schema, idx_t fileIdx, uint64_t maxDefinition,
    uint64_t maxRepeat)
...
void ColumnReader::initializeRead(uint64_t /*rowGroupIdx*/,
    const std::vector<monad_parquet::format::ColumnChunk>& columns,
    monad_apache::thrift::protocol::TProtocol& protocol) {
...
std::unique_ptr<ColumnReader> ColumnReader::createReader(ParquetReader& reader,
    common::LogicalType type, const monad_parquet::format::SchemaElement& schema, uint64_t fileIdx,
...
void ColumnReader::prepareRead(parquet_filter_t& /*filter*/) {
    ...
    defineDecoder.reset();
    block.reset();
    monad_parquet::format::PageHeader pageHdr;
...
void ColumnReader::decompressInternal(monad_parquet::format::CompressionCodec::type codec,
    ...
    case CompressionCodec::SNAPPY: {
        {
            ...
            auto res = monad_snappy::GetUncompressedLength(reinterpret_cast<const char*>(src),
        ...
        auto res = monad_snappy::RawUncompress(reinterpret_cast<const char*>(src), srcSize,
    ...
    case CompressionCodec::ZSTD: {
        auto res = monad_zstd::ZSTD_decompress(dst, dstSize, src, srcSize);
        // LCOV_EXCL_START
        if (monad_zstd::ZSTD_isError(res) || res != (size_t)dstSize) {
    ...
    case CompressionCodec::LZ4_RAW: {
        auto res = monad_lz4::LZ4_decompress_safe(reinterpret_cast<const char*>(src),
...
void ColumnReader::preparePageV2(monad_parquet::format::PageHeader& pageHdr) {
...
void ColumnReader::preparePage(monad_parquet::format::PageHeader& pageHdr) {
    auto& trans = reinterpret_cast<ThriftFileTransport&>(*protocol->getTransport());
...
void ColumnReader::prepareDataPage(monad_parquet::format::PageHeader& pageHdr) {
