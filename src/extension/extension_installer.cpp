...
void ExtensionInstaller::tryDownloadExtensionFile(const ExtensionRepoInfo& repoInfo,
    ...
    httplib::Headers headers = {
        {"User-Agent", common::stringFormat("monad/v{}", MONAD_EXTENSION_VERSION)}};
    ...
    if (!res || res->status != 200) {
        // Network is unavailable or the extension repo cannot be reached. Fall back to creating
        // an empty placeholder so that installation logic can proceed in offline environments.
        fileInfo->syncFile();
        return;
    }
