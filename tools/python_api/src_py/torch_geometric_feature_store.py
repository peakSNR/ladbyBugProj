...
class MonadFeatureStore(FeatureStore):  # type: ignore[misc]
    ...
    def get_all_tensor_attrs(self) -> list[TensorAttr]:
        """Return all TensorAttr from the table nodes, optionally filtered by graph_name."""
        result_list = []
        self.__get_connection()
        allowed_tables: set[str] | None = None
        if self.graph_name is not None:
            # Derive allowed node tables from projected graph relationships.
            rels = self.connection._get_graph_rel_info(self.graph_name)
            allowed_tables = {item["src"] for item in rels} | {item["dst"] for item in rels}
