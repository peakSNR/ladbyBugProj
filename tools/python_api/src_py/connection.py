...
class Connection:
    ...
    def __init__(self, database: Database, num_threads: int = 0):
        """
        Initialise monad database connection.
        ...
        """
        self._connection: Any = None  # (type: _monad.Connection from pybind11)
    ...
    def init_connection(self) -> None:
        ...
        if self._connection is None:
            self._connection = _monad.Connection(self.database._database, self.num_threads)  # type: ignore[union-attr]
    ...
    def _get_graph_rel_info(self, graph_name: str) -> list[dict[str, Any]]:
        """
        Return rel table metadata for a projected graph, including rel_table_id.
        """
        self.init_connection()
        rels = self._connection.get_graph_relinfo(graph_name)
        return [{k: item[k] for k in item.keys()} for item in rels]
    ...
    def neighbor_sample(
        ...
        *,
        ...
        """Sample neighbors from a projected graph.

        Parameters
        ----------
        graph_name : str
            Name of the projected graph to sample from.
        seed_offsets : dict[str, Any]
            Mapping from node table name to an iterable or numpy array of node offsets.
        fanouts : Any
            Sequence of integers describing the fanout per hop. Use -1 to sample full
            neighborhoods.
        replace : bool, default False
            Whether to sample with replacement.
        direction : str, default "fwd"
            Sampling direction. Supports "fwd", "bwd", or "both" (per-hop overrides allowed).
        ...
        return self._connection.neighbor_sample(graph_name, seed_offsets, fanouts, replace, direction, seed, time, time_fields)
