...
class Database:
    ...
    def __init__(
        ...
        *,
        ...
        throw_on_wal_replay_failure: bool
            ...
            Otherwise, Monad will silently ignore the failure and replay up to where the error
            occured.
        ...
        self._database: Any = None  # (type: _monad.Database from pybind11)
        if not lazy_init:
            self.init_database()
    ...
    def get_version() -> str:
        ...
        """
        return _monad.Database.get_version()  # type: ignore[union-attr]
    ...
    def get_storage_version() -> int:
        ...
        """
        return _monad.Database.get_storage_version()  # type: ignore[union-attr]
    ...
    def init_database(self) -> None:
        ...
        if self._database is None:
            self._database = _monad.Database(  # type: ignore[union-attr]
                self.database_path,
    ...
    def get_torch_geometric_remote_backend(
    ...
    ) -> tuple[MonadFeatureStore, MonadGraphStore]:
        """
        ...
        graph_name : str | None
            Name of a projected graph to use for neighbor sampling. If not provided,
            neighbor sampling APIs must be called with an explicit graph name.

        Returns
        -------
        feature_store : MonadFeatureStore
            Feature store compatible with torch_geometric.
        graph_store : MonadGraphStore
        ...
        return (
            MonadFeatureStore(self, num_threads, connection=connection, graph_name=graph_name),
            MonadGraphStore(self, num_threads, graph_name, connection, materialize_edges=False),
        )
