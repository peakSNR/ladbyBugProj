...
class MonadGraphStore(GraphStore):  # type: ignore[misc]
    ...
    def __init__(
        ...
        *,
        ...
        self.materialize_edges = materialize_edges
    ...
    def _get_edge_index(self, edge_attr: EdgeAttr) -> EdgeTensorType | None:
        if not self.materialize_edges:
            raise ValueError(
                "Edge materialization is disabled for this MonadGraphStore instance. "
                "Use MonadGraphStore.neighbor_sample (DB-side sampler) or re-create the "
                "graph store with materialize_edges=True if you explicitly want in-memory COO."
            )
    ...
    def __populate_edge_attrs(self) -> None:
        ...
        if self.graph_name is not None:
            rel_tables = [
                {"name": item["name"], "src": item["src"], "dst": item["dst"]}
                for item in self.connection._get_graph_rel_info(self.graph_name)
            ]
        else:
    ...
    def neighbor_sample(
        self,
        node_table_or_seeds: str | dict[str, Any],
        offsets: np.ndarray | list[int] | None,
        fanouts: list[int],
        *,
        replace: bool = False,
        direction: str = "fwd",
        ...
        """Trigger neighbor sampling through the underlying database connection."""

        if self.graph_name is None:
            raise RuntimeError(
                "neighbor_sample requires a graph name. Provide graph_name when calling "
                "Database.get_torch_geometric_remote_backend()."
            )
        if not self.connection:
            self.connection = Connection(self.db, self.num_threads)

        if isinstance(node_table_or_seeds, dict):
            if offsets is not None:
                msg = "offsets must be omitted when seed dictionaries are provided"
                raise ValueError(msg)
            seed_dict = {
                table: np.asarray(values, dtype=np.int64)
                for table, values in node_table_or_seeds.items()
            }
        else:
            if offsets is None:
                msg = "offsets must be provided when specifying a single node table"
                raise ValueError(msg)
            seed_dict = {node_table_or_seeds: np.asarray(offsets, dtype=np.int64)}
