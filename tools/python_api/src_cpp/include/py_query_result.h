...
public:
    ...
    static py::object convertValueToPyObject(const monad::common::Value& value);

    py::object getAsDF();

    monad::pyarrow::Table getAsArrow(std::int64_t chunkSize, bool fallbackExtensionTypes);
...
private:
    static py::dict convertNodeIdToPyDict(const monad::common::nodeID_t& nodeId);

    void getNextArrowChunk(const std::vector<monad::common::LogicalType>& types,
        const std::vector<std::string>& names, py::list& batches, std::int64_t chunkSize,
        bool fallbackExtensionTypes);
    py::object getArrowChunks(const std::vector<monad::common::LogicalType>& types,
