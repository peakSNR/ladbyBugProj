...
PyDatabase::PyDatabase(const std::string& databasePath, uint64_t bufferPoolSize,
    ...
    monad::extension::ExtensionUtils::addTableFunc<monad::PandasScanFunction>(*database);
    storageDriver = std::make_unique<StorageDriver>(database.get());
    py::gil_scoped_acquire acquire;
    if (monad::importCache.get() == nullptr) {
        monad::importCache = std::make_shared<monad::PythonCachedImport>();
    }
}
