...
struct PyUDFScalarFunction final : ScalarFunction {
    PyUDFScalarFunction(std::string name, std::vector<monad::common::LogicalTypeID> parameterTypeIDs,
        monad::common::LogicalTypeID returnTypeID, scalar_func_exec_t execFunc, scalar_bind_func bindFunc)
...
static LogicalType getLogicalTypeNested(const py::handle& ele) {
    ...
    if (origin.is(py::type::of(py::list()))) {
        ...
        if (py::len(args) != 1) {
            throw monad::common::RuntimeException("List annotations must have exactly one type argument");
        }
        ...
        auto args = ele.attr("__args__");
        if (py::len(args) != 2) {
            throw monad::common::RuntimeException("Dict annotations must have exactly two type arguments");
        }
        auto getitem = args.attr("__getitem__");
...
static LogicalType getLogicalTypeNonNested(const py::handle& ele) {
    ...
    if (ele.is(py::type::of(py::none())) || ele.is(inspect_empty)) {
        ...
        throw monad::common::RuntimeException("List annotations must specify child type");
    } else if (ele.is(py::type::of(py::dict()))) {
        throw monad::common::RuntimeException("Map annotations must specify child types");
    } else {
        throw monad::common::RuntimeException(monad::common::stringFormat("Unsupported annotation of type {}",
...
static py::object getSignature(const py::function& udf) {
    ...
    auto signature_func = monad::importCache->inspect.signature();
...
static scalar_func_exec_t getUDFExecFunc(const py::function& udf, bool defaultNull,
    ...
    return [=](const std::vector<std::shared_ptr<monad::common::ValueVector>>& params,
               const std::vector<monad::common::SelectionVector*>& paramSelVectors,
               monad::common::ValueVector& result, monad::common::SelectionVector* resultSelVector,
               void* /* dataPtr */) -> void {
        py::gil_scoped_acquire acquire;
            ...
            if (defaultNull && hasNull) {
                ...
                try {
                    ...
                    if (catchExceptions) {
                        ...
                        throw monad::common::RuntimeException(e.what());
                    }
                }
            }
        }
