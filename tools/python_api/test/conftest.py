...
import pytest
from test_helper import MONAD_ROOT
...
def init_npy(conn: lb.Connection) -> None:
    ...
    conn.execute(
        f"""
        COPY npyoned from (
          "{MONAD_ROOT}/dataset/npy-1d/one_dim_int64.npy",
          "{MONAD_ROOT}/dataset/npy-1d/one_dim_int32.npy",
          "{MONAD_ROOT}/dataset/npy-1d/one_dim_int16.npy",
          "{MONAD_ROOT}/dataset/npy-1d/one_dim_double.npy",
          "{MONAD_ROOT}/dataset/npy-1d/one_dim_float.npy") BY COLUMN;
        """
    )
    conn.execute(
        """
    ...
    conn.execute(
        f"""
        COPY npytwod FROM (
          "{MONAD_ROOT}/dataset/npy-2d/id_int64.npy",
          "{MONAD_ROOT}/dataset/npy-2d/two_dim_int64.npy",
          "{MONAD_ROOT}/dataset/npy-2d/two_dim_int32.npy",
          "{MONAD_ROOT}/dataset/npy-2d/two_dim_int16.npy",
          "{MONAD_ROOT}/dataset/npy-2d/two_dim_double.npy",
          "{MONAD_ROOT}/dataset/npy-2d/two_dim_float.npy") BY COLUMN;
        """
    )
...
def init_tensor(conn: lb.Connection) -> None:
    ...
    conn.execute(f'COPY tensor FROM "{MONAD_ROOT}/dataset/tensor-list/vTensor.csv" (HEADER=true)')
...
def init_long_str(conn: lb.Connection) -> None:
    ...
    conn.execute(f'COPY personLongString FROM "{MONAD_ROOT}/dataset/long-string-pk-tests/vPerson.csv"')
    conn.execute("CREATE REL TABLE knowsLongString (FROM personLongString TO personLongString, MANY_MANY)")
    conn.execute(f'COPY knowsLongString FROM "{MONAD_ROOT}/dataset/long-string-pk-tests/eKnows.csv"')


def init_tinysnb(conn: lb.Connection) -> None:
    tiny_snb_path = (Path(__file__).parent / f"{MONAD_ROOT}/dataset/tinysnb").resolve()
    ...
    with copy_path.open(mode="r") as f:
        for line in f.readlines():
            line = line.strip()
            line = line.replace("dataset/tinysnb", f"{MONAD_ROOT}/dataset/tinysnb")
            if line:
                conn.execute(line)


def init_demo(conn: lb.Connection) -> None:
    tiny_snb_path = (Path(__file__).parent / f"{MONAD_ROOT}/dataset/demo-db/csv").resolve()
    schema_path = tiny_snb_path / "schema.cypher"
    with schema_path.open(mode="r") as f:
        for line in f.readlines():
            line = line.strip()
    ...
    with copy_path.open(mode="r") as f:
        for line in f.readlines():
            line = line.strip()
            line = line.replace("dataset/demo-db/csv", f"{MONAD_ROOT}/dataset/demo-db/csv")
            if line:
                conn.execute(line)
...
def init_movie_serial(conn: lb.Connection) -> None:
    ...
    conn.execute(f'COPY moviesSerial from "{MONAD_ROOT}/dataset/tinysnb-serial/vMovies.csv"')
