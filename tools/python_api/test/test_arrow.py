...
from monad.constants import ID, LABEL, SRC, DST, NODES
...
def test_to_arrow_map(conn_db_readonly: ConnDB) -> None:
    ...
    pylist = result[0].to_pylist()
    if pylist and not isinstance(pylist[0], dict):
        pylist = [dict(pylist[0])]
    assert pylist == [{"abc": 123, "xyz": 456, "qwe": 781527}]
