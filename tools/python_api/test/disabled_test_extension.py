...
EXTENSION_CMAKE_PREFIX = 'add_definitions(-DMONAD_EXTENSION_VERSION="'
...
def test_extension_install_httpfs(conn_db_readwrite: ConnDB, tmpdir: str, extension_extension_dir_prefix: str) -> None:
    ...
    extension_path = (
        ...
        .joinpath(
            ...
            extension_extension_dir_prefix,
            "httpfs",
            "libhttpfs.monad_extension",
        )
        .resolve()
    )
    opener = urllib.request.build_opener()
    opener.addheaders = [("User-agent", "Monad Test Suite")]
    urllib.request.install_opener(opener)
    download_url = f"http://extension.monaddb.com/v{extension_version}/{extension_extension_dir_prefix}/httpfs/libhttpfs.monad_extension"
    temp_path = Path(tmpdir) / "libhttpfs.monad_extension"
