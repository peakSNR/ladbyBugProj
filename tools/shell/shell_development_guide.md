...
```c++
args::ArgumentParser  parser("MonadDB Shell");
...
After the flags are parsed, the database and connection are set up, and an `EmbeddedShell` object is created. Using this object, we can process every line in the run command startup file (usually `.monadrc`) and then start up the shell, calling `shell.run();`
