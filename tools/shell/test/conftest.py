...
from test_helper import MONAD_EXEC_PATH, MONAD_ROOT, deleteIfExists
...
class ShellTest:
    ...
    def send_finished_statement(self, stmt: str) -> None:
        if self.shell_process:
            assert self.shell_process.expect_exact(["monad", pexpect.EOF]) == 0
...
@pytest.fixture()
def history_path():
    path = os.path.join(MONAD_ROOT, "tools", "shell", "test", "files")
...
@pytest.fixture()
def csv_path():
    return os.path.join(MONAD_ROOT, "tools", "shell", "test", "files", "vPerson.csv")


@pytest.fixture()
def init_path():
    return os.path.join(MONAD_ROOT, "tools", "shell", "test", "files", "start.cypher")
