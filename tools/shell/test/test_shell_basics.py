...
def test_multi_queries_one_line(temp_db) -> None:
    ...
    test = (
        ShellTest()
        .add_argument(temp_db)
        .statement('RETURN "databases rule" AS a; RETURN "monad is cool" AS b;')
    )
    result = test.run()
    result.check_stdout("\u2502 databases rule \u2502")
...
def test_monadrc(temp_db) -> None:
    deleteIfExists(".monadrc")
    # confirm that nothing is read on startup
    test = ShellTest().add_argument(temp_db)
    result = test.run()
    result.check_not_stdout("-- Processing: .monadrc")

    # create a .monadrc file
    with open(".monadrc", "w") as f:
    ...
    result = test.run()
    deleteIfExists(".monadrc")
    result.check_stdout("-- Processing: .monadrc")
    result.check_stdout("maxRows set as 1")
    result.check_stdout("a")

    # create a .monadrc file with errors
    with open(".monadrc", "w") as f:
